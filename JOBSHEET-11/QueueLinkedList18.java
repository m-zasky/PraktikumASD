public class QueueLinkedList18 {
    NodeQueue18 head; // pointer ke node paling depan (yang akan dipanggil duluan)
    NodeQueue18 tail; // pointer ke node paling belakang (tempat masuk antrian baru)
    ...
    void enqueue(MahasiswaaQ18 mhs) {
        if (isFull()) {
            System.out.println("Antrian penuh! Tidak dapat menambahkan data.");
            return;
        }
        NodeQueue18 newNode = new NodeQueue18(mhs, null); // buat node baru
        if (isEmpty()) {
    ...
    MahasiswaaQ18 dequeue() {
        if (isEmpty()) {
            System.out.println("Antrian kosong! Tidak ada mahasiswa yang dapat dipanggil.");
            return null;
        }
        MahasiswaaQ18 dipanggil = head.data; // simpan data head sebelum dihapus
        if (head == tail) {
    ...
    void tampilAntrian() {
        ...
        NodeQueue18 tmp = head; // mulai traverse dari head (depan antrian)
        int no = 1;
